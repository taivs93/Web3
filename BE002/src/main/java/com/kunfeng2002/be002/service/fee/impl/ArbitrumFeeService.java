...
@Service
public class ArbitrumFeeService extends LegacyFeeService {
    public ArbitrumFeeService(Map<String, Web3j> web3Clients) {
        super(getClient(web3Clients, "ARBITRUM"), NetworkType.ARBITRUM);
    }

    private static Web3j getClient(Map<String, Web3j> web3Clients, String key) {
        Web3j client = web3Clients.get(key);
        if (client == null) throw new IllegalStateException("Web3j not found " + key);
        return client;
    }
}
